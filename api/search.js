...
function simpleInterpretation(prompt){
  ...
  return {
    ...
    intentKeys:detectIntentKeys(p),
    maxTravelMinutes:tm?clamp(tm[1],5,90):null,
    travelMode,
...
async function interpretWithAI(prompt){
  ...
  try{
    ...
    return {
      summary:String(j.summary||fallback.summary).slice(0,180),
      intentKeys:keys.length?keys:fallback.intentKeys,
      maxTravelMinutes:j.maxTravelMinutes==null?fallback.maxTravelMinutes:clamp(j.maxTravelMinutes,5,90),
      travelMode:j.travelMode?normalizeMode(j.travelMode):fallback.travelMode,
      openNow:Boolean(j.openNow)
    };
...
async function geoapifySearch(keys,lat,lng,radiusMeters,limit){
  ...
  u.searchParams.set("limit",String(Math.min(50,Math.max(limit*3,18))));
  ...
  return (data.features||[]).map(f=>{
    ...
    return {id:p.place_id||`${la},${lon}`,name,address:p.formatted||p.address_line2||"Address unavailable",lat:la,lng:lon,type:(p.categories||[])[0]||"place",websiteUri:p.website||p.datasource?.raw?.website||null};
  }).filter(Boolean);
}
...
async function overpassSearch(keys,lat,lng,radiusMeters){
  ...
  for(const ep of endpoints){
    try{
      const r=await withTimeout(fetch(ep,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded;charset=UTF-8","User-Agent":"WhatNow/2.0"},body:"data="+encodeURIComponent(query)}),7500);
      ...
      return (data.elements||[]).map(el=>{
        ...
        return {id:`osm-${el.type}-${el.id}`,name,address,lat:la,lng:lon,type:t.amenity||t.leisure||t.shop||t.tourism||"place",websiteUri:t.website||t["contact:website"]||null};
      }).filter(Boolean);
    }catch(e){last=e;}
  }
  throw last||new Error("Overpass unavailable");
}
function dedupeRaw(raw,origin,radiusMiles){
  const seen=new Set(),out=[];
  for(const p of raw){
    if(!Number.isFinite(p.lat)||!Number.isFinite(p.lng)) continue;
    const straightMiles=milesBetween(origin.lat,origin.lng,p.lat,p.lng);
    if(straightMiles>radiusMiles) continue;
    const key=`${String(p.name).toLowerCase()}|${p.lat.toFixed(4)}|${p.lng.toFixed(4)}`;
    if(seen.has(key)) continue; seen.add(key);
    out.push({...p,straightMiles});
  }
  ...
  if(process.env.GEOAPIFY_API_KEY){
    try{
      ...
      const routed=places.map((p,i)=>{
        const cell=row[i]||{};
        const seconds=Number(cell.time),distanceMeters=Number(cell.distance);
...
function finishPlaces(places,origin,mode,maxMinutes,limit){
  const travelmode=mode==="walk"?"walking":mode==="bicycle"?"bicycling":"driving";
  return places
    .filter(p=>Number.isFinite(p.travelMinutes)&&p.travelMinutes<=maxMinutes)
    .sort((a,b)=>a.travelMinutes-b.travelMinutes||a.straightMiles-b.straightMiles)
    .slice(0,limit)
    .map((p,i)=>({
      ...p,
      bestPick:i===0,
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(process.env.GEOAPIFY_API_KEY){
      try{raw=await geoapifySearch(ai.intentKeys,latitude,longitude,radiusMeters,limit);source="Geoapify";}catch(_){warning="Geoapify Places had a problem, so I used the OpenStreetMap backup.";}
    }
    if(raw.length<Math.min(8,limit*2)){
      try{
        const osm=await overpassSearch(ai.intentKeys,latitude,longitude,radiusMeters);
        raw=raw.concat(osm);
    ...
    const candidates=dedupeRaw(raw,origin,radiusMiles);
    const routed=await addRouteTimes(candidates,origin,mode);
    const places=finishPlaces(routed.places,origin,mode,effectiveMinutes,limit);
    ...
    return res.status(200).json({
      ...
      queries:ai.intentKeys,
      maxTravelMinutes:effectiveMinutes,
      travelMode:mode,
      places,
      source,
      routeTimeSource:routed.routeTimeSource,
      warning:places.length?warning:(warning||`No place matched within ${effectiveMinutes} minutes.`),
      fallbackMapUrl,
      fallbackWebUrl,
      needsGeoapifyKey:!process.env.GEOAPIFY_API_KEY,
